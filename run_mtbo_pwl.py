...
print('Running on device: ', tkwargs['device'])
...
for n_offline in variations:
    ...
    for trial in range(trials):
        ...
        for i in range(len(pars_offline_init)):
            ...
            m = 30
            if res_score < m:
                rs = (0.6* (res_score - m)) + m
            else:
                rs = (0.3 * (res_score - m)) + m

            # now we need to add the pars and scores to offline_mt
            scores_offline_mt.append(rs - 0.1*time_score)
    ...
    # save results
    np.save('data/pwl_noffline' + str(n_offline) +'_scores_all_online_mt.npy', scores_all_online_mt)
    np.save('data/pwl_noffline' + str(n_offline) +'_scores_all_offline_mt.npy', scores_all_offline_mt)

    np.save('data/pwl_noffline' + str(n_offline) +'_pars_all_online_mt.npy', pars_all_online_mt)
    np.save('data/pwl_noffline' + str(n_offline) +'_pars_all_offline_mt.npy', pars_all_offline_mt)
