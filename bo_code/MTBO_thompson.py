...
def MTBO_round(scores_online, scores_offline, pars_online, pars_offline, n_online, n_offline, bounds, inequality_constraints, mode="offline"):
    ...
    if mode == "online":
        ...
        thompson_sampling = MaxPosteriorSampling(model, LinearMCObjective(weights=torch.tensor([1.0, 0], **tkwargs)))
        new_x = thompson_sampling(remove_task_feature(train_X_full[-n_offline:]), num_samples=1)
        return new_x.detach().cpu()
